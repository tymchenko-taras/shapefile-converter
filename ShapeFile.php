...
class ShapeFile implements \Iterator
{
    ...
    {
        foreach($this->parseProjectConfig() as $item){
    ...
    /****************************** PRIVATE ******************************/
    private function parseProjectConfig(){
        static $result = null;
        if(is_null($result)){
            $counter = 0;
            $str = str_replace('[', ' => [', $this->prj);
            $str = str_replace('"', '', $str);
            $str = str_replace("'", '', $str);
            $str = preg_replace_callback('#(PARAMETER)#', function($matches) use (&$counter){
                return $matches[1] . $counter++;
            }, $str);
            $str = preg_replace_callback('#([A-Z_]+\d*)#i', function($matches){
                return '"' . $matches[1] . '"';
            }, $str);

            try {
                $result = eval('return array('. $str .');');
                $result = reset($result);
            } catch (ParseError $e) {
                $result = [];
            }
